...
void FR_SPI2_init (void) {
	...
	if (FR_SPI2_get32bitWord (NV_TEST) == 0) {
		//ok prvi test, uradi drugi
		FR_SPI2_set32bitWord (NV_TEST, 0x4D494B49+1);
		if (FR_SPI2_get32bitWord (NV_TEST) == 0x4D494B49+1) {
			//sve OK
			fram_err = 0;
		} 
	}
	if (fram_err) {
		...
		My7segBUFF[4]='r';
		while (1){
			for (int x = 0; x < 8000; x++) {GPIOC->BSRR=GPIO_BSRR_BS13;}
			for (int x = 0; x < 8000; x++) {GPIOC->BSRR=GPIO_BSRR_BR13;}
			handler_led();
		}
	}
